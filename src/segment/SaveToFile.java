...
public class SaveToFile extends SimpleApplication implements ActionListener , AnalogListener{
    ...
    private void CalculateGraph(float r, float im) {
        ...
        for (int i = 1; i < maxcalc; i++)
        {
            ...
            if (data.getReal() > 0)
                realpositivesum += data.getReal();
            else
                realnegativesum += data.getReal();
            
            if (data.getImaginary()> 0)
                imgpositivesum += data.getImaginary();
            else
                imgnegativesum += data.getImaginary();
                        
            finalsum = finalsum.add(data);
            sign *= -1;
        }
        
        hudTextRealPosit.setText("R+ = " + new Float(realpositivesum).toString());
        hudTextRealNeg.setText("R- =" + new Float(realnegativesum).toString());
        
        hudTextImgPosit.setText("I+ = " + new Float(imgpositivesum).toString());
        hudTextImgNeg.setText("I- =" + new Float(imgnegativesum).toString());
        
        hudTextFinalCalc.setText("" + finalsum.getReal() + " + i" + finalsum.getImaginary());
        
        Complex previousplotrealpos = new Complex(0,0);
        Complex previousplotrealneg = new Complex(0,0);
        Complex previousplotrealstep = new Complex(0,0);
        Double stepsumreal = 0.0;
        
        Complex previousplotimgpos = new Complex(0,0);
        Complex previousplotimgneg = new Complex(0,0);
        Complex previousplotimgstep = new Complex(0,0);
        Double stepsumimg = 0.0;
        ...
        for (int i = 1; i < maxcalc; i++)
        {
            ...
            if (plotRorI)
            {
                ...
                if (data.getReal() > 0 )
                {
                    Complex newplot = new Complex(new Double(i).doubleValue()/plotscalex,(data.getReal()/realpositivesum)*plotscaley);
                    rootNode.attachChild(GenerateLine((float)previousplotrealpos.getReal(), (float)previousplotrealpos.getImaginary(), (float)newplot.getReal(), (float)newplot.getImaginary(), i, new ColorRGBA(0.8f, 0.0f, 0.0f,1.0f)));
                    previousplotrealpos = newplot;
                }
                else
                {
                    Complex newplot = new Complex(new Double(i).doubleValue()/plotscalex,(-1.0f*data.getReal()/realnegativesum)*plotscaley);
                    rootNode.attachChild(GenerateLine((float)previousplotrealneg.getReal(), (float)previousplotrealneg.getImaginary(), (float)newplot.getReal(), (float)newplot.getImaginary(), i,new ColorRGBA(0.6f, 0.0f, 0.0f,1.0f) ));
                    previousplotrealneg = newplot;
                }

                Complex newplot = new Complex(new Double(i).doubleValue()/plotscalex,stepsumreal*plotscaley);
                rootNode.attachChild(GenerateLine((float)previousplotrealstep.getReal(), (float)previousplotrealstep.getImaginary(), (float)newplot.getReal(), (float)newplot.getImaginary(), i, ColorRGBA.White ));
                previousplotrealstep = newplot;
                stepsumreal -= data.getReal();
            }
            else
            {
                //-----------imaginary plot-----------
                if (data.getImaginary()> 0 )
                {
                    Complex newploti = new Complex(new Double(i).doubleValue()/plotscalex,(data.getImaginary()/imgpositivesum)*plotscaley);
                    rootNode.attachChild(GenerateLine((float)previousplotimgpos.getReal(), (float)previousplotimgpos.getImaginary(), (float)newploti.getReal(), (float)newploti.getImaginary(), i, new ColorRGBA(0.0f, 0.0f, 0.8f,1.0f)));
                    previousplotimgpos = newploti;
                }
                else
                {
                    Complex newploti = new Complex(new Double(i).doubleValue()/plotscalex,(-1.0f*data.getImaginary()/imgnegativesum)*plotscaley);
                    rootNode.attachChild(GenerateLine((float)previousplotimgneg.getReal(), (float)previousplotimgneg.getImaginary(), (float)newploti.getReal(), (float)newploti.getImaginary(), i,new ColorRGBA(0.0f, 0.0f, 0.6f,1.0f) ));
                    previousplotimgneg = newploti;
                }

                Complex newploti = new Complex(new Double(i).doubleValue()/plotscalex,stepsumimg*plotscaley);
                rootNode.attachChild(GenerateLine((float)previousplotimgstep.getReal(), (float)previousplotimgstep.getImaginary(), (float)newploti.getReal(), (float)newploti.getImaginary(), i, ColorRGBA.White ));
                previousplotimgstep = newploti;
                stepsumimg -= data.getImaginary();
            }
            ...
            {
                rootNode.attachChild(GenerateLine(new Float(i).floatValue()/plotscalex, 100.0f,new Float(i).floatValue()/plotscalex, -100.0f, i, ColorRGBA.Gray));
            }
        }
    }
